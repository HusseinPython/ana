<?php
    ...
    if($do == "edit"){
        ...
        echo '
            ...
            <div>
                ...
                <input style="display:none" type="text" value="'.$article['Img'].'" style="" name="img" />
            </div>
            <input style="display:none" type="text" value="'.$article['Content'].'" style="" name="content" id="content" />
            <div id="editor"></div>
    ...
    if($_GET['do'] == 'update'){      
        echo $_POST['content'];
        if( $_FILES['image']['name'] == $_POST['img'] ){
            $img = $_POST['img'];
        }else{
            $img = $_FILES['image']['name'];
        }
        $stmt = $con->prepare("UPDATE `article` SET `Title` = ?, `Content` = ?, `Img` = ?, `Section` = ?, `Date` = ? WHERE `article`.`ID` = ?");
        $stmt->execute(array($_POST['title'], $_POST['content'], $img, $_POST['section'], date('Y-m-d'), $_GET['id']));
        // header('Location: /anaween/dashboard/index.php?page=news');
        // exit();
    }